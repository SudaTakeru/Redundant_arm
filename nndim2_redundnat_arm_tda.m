...
for t=1:times
    ...
    [Cinputhat,Results2,COut,Cin]=TRR5(output,Cinput,test_output,number_of_redundant,k,k2,k3,ips);
    ...
    for i=1:size(Cinputhat,2)
        inputhat{i}(:,1)=Cinputhat{i}{1}(:,3);
        for j=1:size(Cinputhat{i}{1},1)
            if Cinputhat{i}{1}(j,2)>=0
                inputhat{i}(j,2)=acos(Cinputhat{i}{1}(j,1));
            else
                inputhat{i}(j,2)=-1*acos(Cinputhat{i}{1}(j,1));
            end
        end
    end
    Cin2=cell(size(Cin));
