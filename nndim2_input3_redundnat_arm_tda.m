...
for t=1:times
    ...
    [inputhat,Results2,COut,Cin]=TRR7(output,input,test_output,number_of_redundant,k,k2,k3,ips,r);
    ...
    for i=1:size(inputhat,2)
        signal=[];
        for ij=size(inputhat{i},2)
            signal=[signal;inputhat{i}{ij}];
        end
        ...
        if (i==14 || i==43 || i==45 ) && mod(t,plotf)==0 
            ...
            scatter(inputhat{i}{1}(:,1),inputhat{i}{1}(:,2),'*','r')
            scatter(test_input(i,1),test_input(i,2),'k')
            for ii=1:size(Cin{i},2)
                scatter(Cin{i}{ii}(:,1),Cin{i}{ii}(:,2),'.');
            end
