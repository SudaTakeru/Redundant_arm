% 円を近似できてない…　円問題
% 円はTDAで識別可　クラスター毎にまたTDAするの？？
% 相関係数をみてしまうと識別できないものも…　関数問題 
% ↑例　2次関数　円　sin cos など
% 複雑な関数を識別するのが難しい！！
% 点と線はうまくいく
...
x=-r+2*r*rand(N,1);
y=-r+2*r*rand(N,1);
y1=sqrt(r^2-x.^2/aa^2)*bb^2;
y2=-1*sqrt(r^2-x.^2/aa^2)*bb^2;
x1=sqrt(r^2-y.^2/bb^2)*aa^2;
x2=-1*sqrt(r^2-y.^2/bb^2)*aa^2;
...
datasetb = [data1;data2;data3;data4;data5];
