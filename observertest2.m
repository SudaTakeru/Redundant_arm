...
for j=1:Nt 
    ...
    for ii=1:numcluster(j,1)
        outputx(ii,1)=IDW(clusters{ii}(:,2),clusters{ii}(:,2),clusters{ii}(:,1),testy(j,1),testy(j,1),-2,'ng',k2);
    end
    tx1=sqrt(1-testy(j,1).^2);
    tx2=-1*sqrt(1-testy(j,1).^2);
    if j==1
        trues=[tx1,testy(j,1);tx2,testy(j,1)];
        ...
        scatter(outputx(:,1),testy(j,1)*ones(size(outputx)),'x');
        scatter(trues(:,1),trues(:,2),'*');
        legend('data','y','neighborhood','1','2','output','ture');
    end

    if numcluster(j,1)==2
        error1=(min([tx1;tx2])-min(outputx))^2;
        error2=(max([tx1;tx2])-max(outputx))^2;
        error(ttt,j)=error1+error2;
    else
        error(ttt,j)=NaN;
    end
end
...
outputx2=zeros(Nt,1);
for ii=1:Nt
    outputx2(ii,1)=IDW(data(:,2),data(:,2),data(:,1),testy(ii,1),testy(ii,1),-2,'ng',k2);
    tx1=sqrt(1-testy(ii,1).^2);
    tx2=-1*sqrt(1-testy(ii,1).^2);

    error1=(min([tx1;tx2])-min(outputx2))^2;
    error2=(max([tx1;tx2])-max(outputx2))^2;
    erroridw(ttt,ii)=error1+error2;
end

end

errorrate=zeros(size(error,1),1);
for i=1:size(error,1)
    if ~isnan(error(i,:))
        errorrate(i,1)=sum(error(i,:),2)/size(error,2);
    else
        error00=[];
        for ii=1:size(error,2)
            if ~isnan(error(i,ii))
                error00=[error00;error(i,ii)];
            end
        end
        if ~isempty(error00)
            errorrate(i,1)=sum(error00,1)/size(error00,1);
        else
            errorrate(i,1)=inf;
        end
    end
...
errorrateidw=sum(erroridw,2)/size(erroridw,1);

figure;hold on;
xxx=4*N:4*N:4*N*Tim;
plot(xxx,errorrate(:,1));
plot(xxx,errorrateidw(:,1));
